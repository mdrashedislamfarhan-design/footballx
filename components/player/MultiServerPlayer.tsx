...
import { ChevronLeft, ChevronRight, RefreshCw, AlertCircle, Wifi, ChevronDown, ChevronUp, Settings2 } from 'lucide-react';
...
export default function MultiServerPlayer({ servers, title }: MultiServerPlayerProps) {
  ...
  return (
    <div className="w-full space-y-0">
      {/* ── Player Wrapper ──────────────────────────────── */}
      <div className="relative aspect-video w-full rounded-t-[24px] overflow-hidden bg-[#0c0c12] border border-white/[0.06] shadow-2xl shadow-black/80">
      ...
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-2.5 bg-[#0e0e16] border-x border-white/[0.06] text-xs font-bold text-[#666] hover:text-white transition-colors"
      >
        ...
        <span className="flex items-center gap-1.5 text-[#555]">
          {panelOpen ? <><ChevronUp className="w-3.5 h-3.5" /> Hide Servers</> : <><ChevronDown className="w-3.5 h-3.5" /> Change Server</>}
        </span>
      </button>
